...
<div class="mainindex">
    <h3 id="lastNew">Исторические</h3>
    <hr id="line">
    <?php
    if($_GET['page']!='1') {
        $page = (int)$_GET['page'];
        $querydataPost = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Исторические'ORDER BY`id`DESC LIMIT $page,5");
    }else
        $querydataPost = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Исторические' ORDER BY`id`DESC LIMIT 5");
    $countpost=$row = mysqli_num_rows($allposts = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Исторические'ORDER BY`id`DESC"));
    while ($result = mysqli_fetch_assoc($querydataPost)) {
        ...
        <iframe src=\"https://money.yandex.ru/quickpay/shop-widget?writer=seller&targets=%D0%A4%D0%B8%D0%BB%D1%8C%D0%BC&targets-hint=&default-sum=5&button-text=11&hint=&successURL=http%3A%2F%2Fshop%2Findex.php&quickpay=shop&account=410016651573276\" width=\"450\" height=\"173\" frameborder=\"0\" allowtransparency=\"true\" scrolling=\"no\"></iframe>
    </div>";
    }
    ?>
    <br>
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            <li class="page-item"><a class="page-link" href="http://shop/Sites/Catigories/history.php?page=1">1</a></li>
            <?php
            if($a>$s)
            {
            ...
            for($pageid=5,$s=2;$pageid<$countpost;$pageid+=5,$s++){
                echo "<li class=\"page-item\"><a class=\"page-link\" href=\"http://shop/Sites/Catigories/history.php?page=$pageid\">$s</a></li>";
            }
