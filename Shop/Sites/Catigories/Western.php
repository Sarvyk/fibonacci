...
<div class="mainindex">
    <h3 id="lastNew">Вестерн</h3>
    <hr id="line">
    <?php
    if($_GET['page']!='1') {
        $page = (int)$_GET['page'];
        $querydataPost = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Вестерн'ORDER BY`id`DESC LIMIT $page,5");
    }else
        $querydataPost = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Вестерн' ORDER BY`id`DESC LIMIT 5");
    $countpost=$row = mysqli_num_rows($allposts = mysqli_query($connect, "SELECT * FROM `Films`WHERE `style`='Вестерн'ORDER BY`id`DESC"));
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            <li class="page-item"><a class="page-link" href="http://shop/Sites/Catigories/Western.php?page=1">1</a></li>
            <?php
            if($a>$s)
            {
            ...
            for($pageid=5,$s=2;$pageid<$countpost;$pageid+=5,$s++){
                echo "<li class=\"page-item\"><a class=\"page-link\" href=\"http://shop/Sites/Catigories/Western.php?page=$pageid\">$s</a></li>";
            }
